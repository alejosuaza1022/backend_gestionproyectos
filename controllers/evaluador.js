...
let obtener_en_espera_evaluar = async(req, res) => {
    let servicio = new s_pg();
    let id_evaluador = req.params.id
    let sql = ' select acc_usuarios.nombre as nombre, pu_publicacion_revision.id,pu_propuestas_publicaciones.id as idpub,titulo,area,facultad,tipo_publicacion,archivo,id_autor,correo from pu_propuestas_publicaciones inner join pu_publicacion_revision on pu_propuestas_publicaciones.id = id_publicacion inner join pu_autores_publicaciones on pu_propuestas_publicaciones.id = pu_autores_publicaciones.id_publicacion  inner join acc_usuarios on acc_usuarios.id = id_autor where estado = 0 and id_evaluador = $1;'
    await servicio.eje_sql(sql, [id_evaluador]).then(bd_res => {
        res.status(200).send({
            publicaciones: bd_res.rows,
            message: "exitoso"
