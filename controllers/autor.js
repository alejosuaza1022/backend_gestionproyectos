...
let obtener_estado_publicacion = async(req, res) => {
    ...
    let sql = 'select  pu_propuestas_publicaciones.id as id_publicacion,fecha_subida,pu_publicacion_revision.id as id_revision,retroalimentacion,estado as est,case when estado = 1 then fecha_realizada + 15  else null end as plazo_maximo ,estadoPublic(estado) as estado,titulo,(select id_evaluador  from pu_publicacion_revision where id_publicacion = $1 limit 1) as id_evaluador from pu_publicacion_revision right join pu_propuestas_publicaciones on pu_propuestas_publicaciones.id = pu_publicacion_revision.id_publicacion inner join pu_autores_publicaciones on pu_autores_publicaciones.id_publicacion =pu_propuestas_publicaciones.id where pu_propuestas_publicaciones.id=$1 and id_autor=$2 order by fecha_realizada desc limit 1;'
